...
export class PostResolver {
	...
	async favoritePost(
		...
		const post = await Post.getForAction(input.id, user);
	...
	@FieldResolver()
	async text(@Root() root: Post, @Ctx() { user }: AuthorizedContext) {
		if (await root.shoudDisplayPreview(user)) {
			return truncate(root.text, { length: 30 });
		}
